#
# Default make builds both original darknet, and share libary
# make darknet - only darknet (original code), OPENCV=0
# make darknet-shared - build the shared-lib version (without darknet.c calling wrapper), OPENCV=1
# 
...
OBJS_SHARED = $(addprefix $(OBJDIR_SHARED), $(OBJ-SHARED))

all: backup obj results $(EXEC) $(SHARED_LIB)
...
$(SHARED_LIB): obj clean $(OBJS_SHARED)
	$(CC_CPP) $(COMMON) $(CFLAGS) $(OBJS_SHARED) -o lib$@.so $(LDFLAGS) -shared	

$(OBJDIR_SHARED)%.o: %.c $(DEPS)
	$(CC_CPP) $(COMMON) $(CFLAGS_CPP) $(CFLAGS) -fPIC -c $< -o $@

$(OBJDIR)%.o: %.cu $(DEPS)
	$(NVCC) $(ARCH) $(COMMON) --compiler-options "$(CFLAGS)" -c $< -o $@

$(OBJDIR_SHARED)%.o: %.cu $(DEPS)
...
clean:
	rm -rf $(OBJS) $(EXEC) $(SHARED_LIB) $(OBJS_SHARED)
